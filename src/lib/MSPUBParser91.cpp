...
ColorReference MSPUBParser91::getColor(int colorId) const
{
  ...
  // 0: black, 1: white, 2: red, 3: green, blue, yellow, cyan magenta
  unsigned const colors[]= {0x000000, 0xffffff, 0x0000ff, 0x00ff00, 0xff0000, 0x00ffff, 0x007f00, 0xc000c0};
...
bool MSPUBParser91::parseShape(librevenge::RVNGInputStream *input, BlockInfo91 const &info)
{
  ...
  if (patternId && type!=0) // fixme
    m_collector->setShapeFill(info.m_id, std::make_shared<SolidFill>(ColorReference(getColor(colors[0])), 1, m_collector), false);
  return true;
}
